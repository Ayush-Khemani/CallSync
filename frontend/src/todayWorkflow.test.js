...
test('builds a daily queue for upcoming meetings, booking follow-ups, missing outcomes and due next actions', () => {
  const meetings = [
  ...
  expect(workspace.actions.map((item) => item.id)).toEqual([4]);
  expect(todayAttentionCount(workspace)).toBe(3);
});
...
test('does not treat healthy pending requests or future meetings beyond 24 hours as immediate attention', () => {
  ...
  expect(todayAttentionCount(workspace)).toBe(0);
});
